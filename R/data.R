...
loadPDXData<-function(reticulate_python=NULL){
  ...
  icyteData<<-dataFromSynTable(data.tab, syn, 'Incucyte drug Data')%>%
    ...
    ungroup()
  mt.meta <- syn$tableQuery('SELECT id,individualID,modelSystemName,experimentalCondition,parentId FROM syn21993642 WHERE "dataType" = \'drugScreen\' AND "assay" = \'cellViabilityAssay\' AND "fileFormat" = \'csv\' AND "parentId" not in (\'syn26433454\',\'syn25791480\',\'syn25791505\',\'syn26433485\',\'syn26433524\')')$asDataFrame()
