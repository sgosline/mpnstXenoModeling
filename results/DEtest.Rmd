...
#library(devtools)
if(!require(MXM))
    devtools::install_github('sgosline/MXM')
library(MXM)
```
...
design_matrix_and_limma_analysis <- function(counts) {
    ...
    design <- model.matrix(~0 + qual + age + sex + group)
  ...
  #  names <- as.numeric(gsub("[^0-9.]", "", names))
    subset.names <- colnames(counts) #sampleIDs[names]
...
get_topGenes_and_plot_heatmap <- function(fit,dge,coefname,heatmapFN) {
    ...
    topGenes <- topTable(fit,coef=c(coefname),p.value=0.01,number=100,lfc=1)
...
topGenes <- get_topGenes_and_plot_heatmap(fit,dge,'Good','DE_GoodTissue_sex_age.pdf')
topGenes <- get_topGenes_and_plot_heatmap(fit,dge,'','DE_GoodTissue_sex_age.pdf')
```
...
gseout <- do_gseGO_of_topGenes(fit, dge,'Male','ranked.gene.list.Male.tsv', 'gsego_Male.tsv')
