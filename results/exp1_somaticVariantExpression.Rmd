...
<<<<<<< HEAD:results/exp1_somaticVariantExpression.Rmd
varData <- mpnstXenoModeling::loadVariantData()
rnaSeq <- mpnstXenoModeling::loadRNASeqData()
=======
>>>>>>> master:results/fig1_somaticVariantExpression.Rmd
...
all.res <- lapply(intersect(genelist, varData$Symbol), function(x) {
  ...
  gs.res <- mut.res %>%
    ...
    doGSEA(., prefix = paste0('GSEA_mutated', x, compress_output=FALSE))
    
  gsea_compressed <- mut.res %>%
    tibble::rownames_to_column('GENEID') %>%
    left_join(pmap) %>%
    dplyr::select(Gene = 'GENENAME', value = 'log2FoldChange') %>%
    doGSEA(., prefix = paste0('GSEA_mutated_compressed', x, compress_output=TRUE))
  ...
  if (nrow(sig.genes) > 0)
    go.res <- doRegularGo(sig.genes$GENENAME,
                          prefix = paste0('GO_mutated', x))
  
    compressed_go <- doRegularGo(sig.genes$GENENAME,
