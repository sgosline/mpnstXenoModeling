...
tgi.mat<-mtStats%>%
  ...
  pheatmap(tgi.mat,cellwidth = 10,cellheight=10,main='IC50 across MT data')
...
p<-ggplot(subset(mtStats,Drug%in%mk.diff.drugs$Drug),aes(x=Drug,y=maxKilling,fill=CellLine))+
    geom_bar(stat='identity',position='dodge')+
  scale_x_discrete(guide = guide_axis(angle=90))+ggtitle("Max Killing of drugs with sufficient samples at 0.6")
#ggsave(paste0('drugsWith2SensAt',auc.thresh,'.png'),p)
p
```
...
full.res%>%group_by(Drug)%>%
  summarize(sigGenes=count(padj<0.05))%>%
  DT::datatable()                                  
```
...
all.res<-lapply(mt.diff.drugs$Drug,function(x){
  sens.pats <- subset(mt.sens.class,Drug==x)%>%subset(sens==TRUE)
  res.pats <- subset(mt.sens.class,Drug==x)%>%subset(sens==FALSE)
