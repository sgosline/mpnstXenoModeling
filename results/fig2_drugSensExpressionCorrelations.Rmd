...
good.mt <- subset(clin.tab,MicroTissueQuality%in%c('Good','Robust'))%>%distinct()
...
```{r rnaseq,message=FALSE,warning=FALSE}
 ...
 library("EnsDb.Hsapiens.v86")
  ...
  def = umap::umap.defaults
  def$n_neighbors=5
  ##CREATE dataframe of gene expression manifold mapping
  ex.df<-umap::umap(t(log10(0.01+genemat)),config=def,method='naive',preserve.seed=TRUE)$layout%>%
    as.data.frame()%>%
...
all.res<-lapply(unique(eval.drugs),function(x){
  ...
  gs.res<-drug.res%>%
    ...
    dplyr::select(Gene='GENENAME',value='log2FoldChange')%>%doGSEA(.,prefix=paste0('GSEA',x))
...
There is not a lot of enrichment here from the GSEA persecptive. 