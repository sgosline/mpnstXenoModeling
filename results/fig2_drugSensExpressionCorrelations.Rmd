...
drug.plots <- lapply(unique(minDrugs$drug),function(dname){
  drugStats%>%subset(individualID!='WU-545')%>%
    subset(drug==dname)%>%ggplot(aes(x=AUC,y=TGI))+geom_text(aes(label=Sample))+ggtitle(dname)
})
...
drug.plots <- lapply(singles,function(dname){
  mtStats%>%
    subset(Drug==dname)%>%ggplot(aes(x=maxKilling,y=auc))+geom_point()+ggtitle(dname)
})
...
all.res<-lapply(diff.drugs$Drug,function(x){
  ...
  mpnstXenoModeling::plotTopGenesHeatmap(drug.res, dds, pmap,x,patients=c(sens.pats$CellLine,res.pats$CellLine),adjpval=0.01, upload=FALSE, parentID='syn25323411')
