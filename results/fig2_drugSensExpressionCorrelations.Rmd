...
cudc<-grep("CUDC",mtStats$Drug)
mtStats$Drug[cudc]<-rep("CUDC-907",length(cudc))

##fig drug combo errors
sv<-grep("Selumetinib;Vorinost",mtStats$Drug)
mtStats$Drug[sv]<-rep('Selumetinib;Vorinostat',length(sv))

mt<-grep('Trabectedin;Mirdametinib',mtStats$Drug)
mtStats$Drug[mt]<-rep('Mirdametinib;Trabectedin',length(mt))

ot<-c(grep('Trabectedin; Olaparib',mtStats$Drug),grep('Trabectedin;Olaparib',mtStats$Drug))
mtStats$Drug[ot]<-rep('Olaparib;Trabectedin',length(ot))
...
eval.drugs<-c('Mirdametinib','Mirdametinib;Trabectedin','Trabectedin','Olaparib','Olaparib;Trabectedin')
...
```{r rnaseq,message=FALSE,warning=FALSE}
  ...
  genemat<-counts(dds,normalize=TRUE)%>%
    ...
    tidyr::pivot_longer(-GENEID,names_to='Sample',values_to='counts')%>%
    left_join(pmap)%>%
  ...
  ex.df<-umap::umap(t(genemat),method='naive',preserve.seed=TRUE)$layout%>%
    as.data.frame()%>%
...
ggsave("UMAP_GeneExpression.png",p)
  pmap<-pmap%>%
...
all.res<-lapply(unique(eval.drugs),function(x){
  ...
  p
  ggsave(paste0('umapBy',x,'AUC.png'),p)
...
go.res<-lapply(all.res,function(drug.res){
    ...
    res2<-mpnstXenoModeling::doRegularGo(genes,prefix=paste0('AUC',x)) 
...
mk.all.res<-lapply(eval.drugs,function(x){
  ...
  p
  ggsave(paste0('umapBy',x,'MK.png'),p)
...
write.table(full.res,file='maxKillMTGenesAtMidMK.csv')
