...
drug.list<-list(MirdaTrab=c('Mirdametinib','Mirdametinib;Trabectedin','Trabectedin'),
                ...
                ShpRibo=c('SHP 99.0','SHP099;Ribociclib','Ribociclib'),
                ShpTram=c('SHP 99.0','Trametenib','SHP099;Trametinib'))
...
lapply(names(drug.list),function(drugGroup){
  ...
  ggsave(paste0('SampleMT',drugGroup,'plots.png'),
         cowplot::plot_grid(plotlist = samp.plots,ncol=2),height=12)


  drug.plots<-lapply(intersect(eval.drugs,mtDrugData$DrugCol),
                     function(drug) plotMTTreatmentBySample(mt.subs,drug))

  ggsave(paste0('DrugMT',drugGroup,'plots.png'),
         cowplot::plot_grid(plotlist = drug.plots,ncol=2),height=12)
})
