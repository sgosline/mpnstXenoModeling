...
good <-rownames(subset(var.ID,MicroTissueQuality%in%c('Good','Robust','Usable')))
...
mpnstXenoModeling::plotTopGenesHeatmap(qual_res, dds, pmap,'MTqual',patients=c(good,bad),adjpval=0.0001, upload=FALSE, parentID='syn25323411')


res<-pmap[rownames(subset(qual_res,padj<0.0001)),'GENENAME']%>%
  doRegularGo(prefix='MTqualGO')
plot(res)
```
