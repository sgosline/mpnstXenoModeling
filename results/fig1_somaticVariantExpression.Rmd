...
varData <- mpnstXenoModeling::loadVariantData(syn)
...
mutMat<-varData%>%
  ...
  subset(Symbol!="")%>%
  dplyr::select(-c(individualID,specimenID,synid,Age,Sex,MicroTissueQuality,Location,Size,Clinical.Status,AD))%>%
  distinct()%>%
  tidyr::pivot_wider(names_from=Sample,values_from=Tumor_AF,values_fn=list(Tumor_AF=max),
                     values_fill=list(Tumor_AF=0.0))%>%
    tibble::column_to_rownames('Symbol')

#mutMat$SymbolUnique<-make.names(mutMat$Symbol,unique=TRUE)
#mutMat<-mutMat%>%tibble::column_to_rownames('SymbolUnique')%>%dplyr::select(-Symbol)
...
smutMat<-mutMat[intersect(genelist,allgenes),]#DT_result[result==TRUE,]$allgenes,rownames(annotes)]
...
```{r recurrent mutations, results="asis", message=FALSE, warning=FALSE}

topMuts=dplyr::filter(varData,Tumor_AF>0)%>%
  subset(!is.na(Symbol))%>%
  subset(Symbol!="")%>%
    group_by(Symbol)%>%
...
all.res<-lapply(intersect(genelist,varData$Symbol),function(x){
  ...
  ##add col data to dds
  mut.res <- ds2FactorDE(dds, ids1=modSamps,
                                    ids2=wtSamps,name=x,doShrinkage=TRUE)
  
  if(nrow(mut.res)>0)
    try(mpnstXenoModeling::plotTopGenesHeatmap(mut.res, dds,
                                           pmap,                                        
                                           paste0('mutated_',x),
  ...
  #  left_join(tibble::rownames_to_column(pmap,'GENEID'))%>%
    dplyr::select(Gene='GENENAME',value='log2FoldChange')%>%doGSEA(.,prefix=paste0('GSEA_mutated',x))
  ...
  if(nrow(sig.genes)>0)
    go.res<-doRegularGo(sig.genes$GENENAME,prefix=paste0('GO_mutated',x))
  ...
  sumtab<-full.res%>%subset(padj<0.05)%>%group_by(Mutation)%>%summarize(count=n())
  DT::datatable(sumtab)
```
