...
Pattern = paste(genelist, collapse="|")

DT_result <- data.table(allgenes, result=grepl(Pattern, allgenes))

```

```{r selected muts, warning=FALSE, results="asis"}

missed.genes<-DT_result[result==FALSE,]$allgenes
print(paste('Missing mutations in genes',paste(missed.genes,collapse=',')))

smutMat<-mutMat[DT_result[result==TRUE,]$allgenes,rownames(annotes)]
...
```{r}
rnaSeqm<-merge(trmutMat%>%as.data.frame()%>%
                 tibble::rownames_to_column('Sample'),rnaSeq,by='Sample')
...
```{r}
dds <- deseq2NormFilter(counts, coldata, "~Sex")
...
sigs <-filtered.res%>%
  dplyr::select(GENEID,GENENAME)%>%tibble::column_to_rownames('GENEID')
...
```{r}
dds <- deseq2NormFilter(counts, coldata, "~Clinical.Status")
filtered.res <- filterDDS(dds,pmap,adjPVal=0.05)
```
...
```{r}
dds <- deseq2NormFilter(counts, coldata, "~Age")
filtered.res <- filterDDS(dds,pmap,adjPVal=0.05)
```
...
```{r}
dds <- deseq2NormFilter(counts, coldata, "~MicroTissueQuality")
filtered.res <- filterDDS(dds,pmap,adjPVal=0.05)
```
