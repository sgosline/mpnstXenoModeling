...
```{r}
# Averaging all occurrences for non-unique Symbol
mutMat <- setDT(mutMat)[,lapply(.SD, mean), by=.(Symbol)]
```
...
mutMat<-mutMat%>%tibble::column_to_rownames('Symbol')
...
Pattern = paste(paste0('^',genelist,'$'), collapse="|")
