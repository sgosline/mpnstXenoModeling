...
We now want to evaluate the behavior of single drugs to determine which ones were anti-correlated. Are drugs that are anti-correlated more likely to be synergistyc? 


```{r correlation analysiss,message=FALSE,warning=FALSE}
auc.mat<-mtStats%>%
  ...
  pheatmap(cor(t(auc.mat[singls,])))

  cortab<-cor(t(auc.mat[singls,]))%>%as.data.frame()%>%
    tibble::rownames_to_column('firstDrug')%>%
    pivot_longer(cols=c(-firstDrug),values_to='corVal',names_to='otherDrug')%>%
    arrange(corVal)%>%
    unite(c(firstDrug,otherDrug),col='combo',sep=';')%>%
    rowwise()%>%
    mutate(combo=paste(sort(unlist(strsplit(combo,split=';'))),collapse=';'))%>%
    dplyr::select(combo,corVal)%>%distinct()
  
  mt.combs<-comb.stats%>%
    unite(Drug1,Drug2,sep=';',col='combo')%>%dplyr::select(combo,nSyn)%>%distinct()

  cortab%>%arrange(corVal)%>%
    inner_join(mt.combs)%>%
    ggplot(aes(x=nSyn,y=corVal,col=nSyn))+geom_point()
```
### RNA Expression
