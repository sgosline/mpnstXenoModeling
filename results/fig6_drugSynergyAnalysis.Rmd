...
combined<-comboStats%>%
  left_join(rename(singleStats,Drug1='Drug',auc1='auc'))%>%
  left_join(rename(singleStats,Drug2='Drug',auc2='auc'))%>%
...
singleStats<-mtStats%>%subset(Drug%in%unlist(cdrugs))%>%dplyr::select(Drug,CellLine,maxKilling)
...
mk.comb.stats<-combined%>%group_by(Drug1,Drug2)%>%
  mutate(nSynMK=count(mksynergistic))
...
auc.mat<-mtStats%>%
  ...
  ggsave('correlationDotPlot.pdf',width=12)
  cortab%>%
    left_join(mt.combs)%>%    
  ...
  ggsave('correlationBarplot.pdf',width=12)
