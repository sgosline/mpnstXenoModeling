...
syn<-loadPDXData()#reticulate::import('synapseclient')$login()
#data.tab<-syn$tableQuery('select * from syn24215021')$asDataFrame()
rnaSeq<<-mpnstXenoModeling::loadRNASeqData(syn)
  #mpnstXenoModeling::dataFromSynTable(data.tab,syn,'RNASeq')%>%
...
res=mpnstXenoModeling::plotCorrelationEnrichment(as.matrix(data.mat), keggpaths, 
                                                 fdr.cutoff = 0.05, 
...
plotTopGenesHeatmap(qual_res, dds, pmap,'MTqual',
                    patients=c(good,bad),newVar='',adjpval=0.0001, upload=TRUE, parentID='syn25323411')

res<-pmap%>%
  subset(GENEID%in%rownames(subset(qual_res,padj<0.0001)))
...
plotTopGenesHeatmap(qual_res, dds, pmap,'MTqualShrink',patients=c(good,bad),adjpval=0.0001, upload=TRUE, parentID='syn25323411')
