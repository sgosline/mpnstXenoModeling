...
library(pheatmap)
 ...
 pheatmap(mutMat,clustering_distance_cols = 'correlation',cellwidth = 10,annotation_col = annotes,labels_row = rep("",nrow(mutMat)),labels_col=rep("",ncol(mutMat)),filename='allMutations.pdf')
```

Now we can confirm the variants cluster by sample.

```{r main mutations,echo=FALSE}
topMuts<-names(which(apply(mutMat,1,function(x) length(which(x>0)))>3))
 
 pheatmap(mutMat[topMuts,],cellwidth = 10,cellheight=10,annotation_col = annotes,labels_col=rep("",ncol(mutMat)),filename='recMutations.pdf')
