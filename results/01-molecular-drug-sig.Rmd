...
```{r mutated gene expression,echo=FALSE,warning=FALSE}
 rnamat<-rnaSeq%>%
  subset(!is.na(Symbol))%>%
  subset(!is.na(NumReads))%>%
  dplyr::select(-c(Sample,Sex,TXID,TPM,Clinical.Status,Size,Location,Age,MicroTissueQuality))%>%
  distinct()%>%#View()
  tidyr::pivot_wider(names_from=synid,values_from=NumReads,
                     values_fn=list(NumReads=mean),values_fill=list(NumReads=0.0))%>%
    tibble::column_to_rownames('Symbol')
