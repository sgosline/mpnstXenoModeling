...
topMuts<-c("TP53",names(which(apply(mutMat,1,function(x) length(which(x>0)))>3)))
 pheatmap(log10(0.01+mutMat[topMuts,]),cellwidth = 10,cellheight=10,annotation_col = annotes,labels_col=rep("",ncol(mutMat)))
 pheatmap(log10(0.01+mutMat[topMuts,]),cellwidth = 10,cellheight=10,annotation_col = annotes,labels_col=rep("",ncol(mutMat)),filename='recMutations.pdf')
