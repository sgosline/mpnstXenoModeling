...
```{r get sig,warning=FALSE,results="asis"}
res<-pmap[rownames(subset(sex_res,padj<0.05)),'GENENAME']%>%doRegularGo(prefix='sex')
```

Lastly we can rank the genes and do Gene Set Enrichment.

```{r getGSEA,warning=FALSE,results="asis"}
...
```{r analysingExist, echo=FALSE,warning=FALSE,results="asis"}
...
mpnstXenoModeling::plotTopGenesHeatmap(exist_res, dds, pmap,'clin',adjpval=0.05, upload=FALSE, parentID='syn25323411')


res<-pmap[rownames(subset(exist_res,padj<0.05)),'GENENAME']%>%doRegularGo(prefix='prognosisDiff')
```
```{r analysis exist gsea,echo=FALSE, warning=FALSE}
genes.with.values<-cbind(exist_res,pmap[rownames(exist_res),])%>%
  dplyr::select(Gene='GENENAME',value='log2FoldChange')
                   
res2=doGSEA(genes.with.values,prot.univ=NULL,prefix='prognosisDiff',useEns=FALSE)
```
...
```{r analysingAge, echo=FALSE,warning=FALSE,results="asis"}
age_res <- mpnstXenoModeling::ds2FactorDE(dds,
                                    subset(var.ID,Age>18)$Sample,
                                    subset(var.ID,Age<19)$Sample,'Age')

mpnstXenoModeling::plotTopGenesHeatmap(age_res, dds, pmap,'age', adjpval=0.05, upload=FALSE, parentID='syn25323411')


res<-pmap[rownames(subset(age_res,padj<0.05)),'GENENAME']%>%doRegularGo(prefix='age')
```
```{r analysis age gsea, echp=FALSE, results="asis",warning=FALSE}
...
```{r analysingQual, echp=FALSE, results="asis",warning=FALSE}
...
mpnstXenoModeling::plotTopGenesHeatmap(qual_res, dds, pmap,'MTqual',adjpval=0.001, upload=FALSE, parentID='syn25323411')


res<-pmap[rownames(subset(qual_res,padj<0.001)),'GENENAME']%>%doRegularGo(prefix='MTqual')
```

```{r analysis qual gsea,results="asis"}
